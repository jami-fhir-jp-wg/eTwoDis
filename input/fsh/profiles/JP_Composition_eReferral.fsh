...
* section.code.coding.system = "http://jpfhir.jp/fhir/eReferral/CodeSystem/document-section" (exactly)
...
* section[remarksCommunicationSection].text ^definition = "本セクションの内容をテキストで表現した文字列。内容を省略しても構わない。"
...
* section[remarksCommunicationSection].entry only Reference(DocumentReference or  JP_DocumentReference)  
...
* section[pdfSection].entry only Reference(DocumentReference or JP_DocumentReference)
