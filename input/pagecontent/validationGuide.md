...
### Bundleリソースデータとその構成リソースデータのValidation方法について
...
#### 手順（準備編）　：　Validation対象に関わらず一度だけ準備が必要な手順
　
...
以下の３つのパッケージをOS種別にかかわらずダウンロードしする。(Windowsの場合も拡張子tgzのファイル）。ダウンロード後の解凍はしない。なお、ファイル名中のr4-x.x.xのバージョン番号部分はダウンロード時期により異なる。
 ...
 　 
  - jp-eReferral.r4　パッケージ
    ...
    - tgz形式 : [https://jpfhir.jp/fhir/eReferral/jp-eReferral.r4-1.1.6.tgz](https://jpfhir.jp/fhir/eReferral/jp-eReferral.r4-1.1.6.tgz)
...
Validationコマンドのパラメータ説明
  ...
  - -ig [pkgJp]/jp-eReferral.r4-1.1.6.tgz : ３文書のひとつである、診療情報提供書仕様に従ったBundleリソースのValidationのためのプロファイル等を格納したパッケージ。必須。
